...
var basket = new function() {
    ...
    this.update = function(d) {
        if (d.success && this.hash == d.hash) {
            switch (d.mode) {
                ...
                case 'big'	: { $target = $('#SHOPPING_CART'); break; }
            }
            ...
            $('div.number.discount').html(d.discount);
            global.hideStatus('loading');
        }
    };

    /**
     * Update basket count on the icon cart
     * @modify Staniolsav web
     * @param id
     * @param object items
     */
    this.update_sizes_button = function(id, items) {

        var count   = ObjectLength(items);

        if(!this.isNumber(id))
           var id      =   parseInt(id.replace("sizes_",""));

        var sizes_button = $('#sizes_button_'+id);

        var btnval = parseInt(sizes_button.html());
        ...
        if(sizes_button.length)
        {
            if(count > 0)
            {
                ...
                {
                    btnval =   btnval.trim();
                    sizes_button.addClass('on').html(count+btnval);
                }
                else
                    sizes_button.addClass('on').html(count);
            }
            else
    ...
    this.addToBasketSingle = function(product_id)
    {
       ...
       items.push({
        ...
        this.addToBasket(product_id, true, items);
    }
            ...
            if (itemsQueryString.length) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        //@sweb fix update basket button
                        then.update_sizes_button(containerId, items);
                        return basket.update(data);
                    },
                    dataType: 'json',
                    timeout: 20000,
