<?php
	...
	class Categories extends \Phalcon\Mvc\Model
	{
		...
		public function parseRemap($shopID = 1, $url = false, $limit = 100, $cache = false){
			...
			if ($result === null) {
				if($url != false){
					...
					//Забираем категории
					//@deprecated Для этого можно сделать исключение по $this->_shopCategories
					$sql = "SELECT c.id
					FROM ".Common::TABLE_CAT_SHOP_REL." csr
				...
				// ИДЕЯ кроется сдесь )) Вырезать главные категории, имея ввиду что главная - это первый элемент
				// но можно же сделать исключение из $this->_shopCategories и не формировать массив
				...
				if(is_array($tags['tags'][$nameCategory]['items'])){
					foreach($tags['tags'][$nameCategory]['items'] as $count => $tagInfo){
						foreach($tagInfo as $itemTagInfo){
							if(isset($filter['tags']) && in_array($itemTagInfo['tag_id'],$filter['tags'])){
		...
		public function renderItemsLine($filter = array(), $price_id = 1, $cache = false)
		{
			...
			FROM `".Products::PRODUCT_RELATION."` pr
			INNER JOIN `".Products::TABLE."` p ON (p.id = pr.product_id)
			INNER JOIN `".Prices::TABLE."` price ON (p.id = price.product_id && price.id = ".(int)$price_id." )
			-- INNER JOIN `".Categories::TABLE."` cat ON (cat.id = pr.category_id)
			INNER JOIN `".Brands::TABLE."` brand ON (brand.id = p.brand_id)";
				if(isset($filter['top'])){
					$sql .= 'INNER JOIN (SELECT id FROM products as products ';
					if(isset($filter['sex'])){
				...
				if(isset($filter['category'])){
					//$sql .= ' AND (cat.parent_id IN ('.implode(',',$filter['category']).') OR pr.category_id IN ('.implode(',',$filter['category']).')) ';
