...
class CatalogueController extends ControllerBase
{
	...
	public function itemAction()
	{
		...
		if($item && !empty($item))
		{
			...
			// доавим в сессию как просмотренный товар
			if($this->session->has('visited_items'))
				$visited	=	$this->session->get('visited_items');
			$visited[$item['product_id']]	=	time();
			$this->session->set("visited_items", $visited);
	...
	public function brandsAction()
	{
		...
		$arrayBrand = Catalogue::arrayToAssoc((array)$this->brandsModel->getAllBrands($this->_shop['price_id']), 'name');
		$result = '';
