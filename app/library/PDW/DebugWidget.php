...
class DebugWidget implements \Phalcon\DI\InjectionAwareInterface
{
	...
	public function getInsertScripts()
	{
		...
		$css = array('/assets/pdw-assets/style.css', '/assets/pdw-assets/lib/prism/prism.css');
		foreach ($css as $src) {
			$link = $url->get($src);
		...
		$js = array(
				'/assets/pdw-assets/jquery.min.js',
				'/assets/pdw-assets/lib/prism/prism.js',
				'/assets/pdw-assets/pdw.js'
		);
		foreach ($js as $src) {
			$link = $url->get($src);
			$link = str_replace("//", "/", $link);
