...
class Navigation  extends \Phalcon\Translate\Adapter\NativeArray {
    ...
    public function __construct($config)  {
        ...
        foreach ($this->_config->navigation as $navigationName => $conf) {
    ...
    public function toHtml($name) {
        $helper = new \Navigation\View\Helpers\Navigate();
    ...
    public function mapNode($config) {
        ...
        $node = new \Navigation\Navigation\Node();
     ...
     * @return void
     */
    public function setActiveNode($action, $controller) {
        $this->dissactiveNodes();

        foreach ($this->_navigations as $navigation) {
            $this->_activeCollection($navigation->getChilds(), $action, $controller);
        }
    }
     ...
     * @return void
     */
    public function _activeCollection($collection, $action, $controller) {
        ...
        foreach ($collection as $node) {
            ...
            if ($node->hasChilds())
                $this->_activeCollection($node->getChilds(), $action, $controller);
        }
    }
